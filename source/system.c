...
//Перезагрузка счётчика сторожевого таймера
void IWDG_Reload()
{
  IWDG->KR = 0xAAAA;
}
...
//################## Функция нейтрализации дребезга контактов ##################
...
//################# Функция расстановки приоритетов прерываний #################
void InterruptsPrioritySet()
{
  //NVIC_SetPriority(SPI1_IRQn, 10);
  //NVIC_SetPriority(EXTI1_IRQn, 13);
  NVIC_SetPriority(TIM2_IRQn, 15);          //Таймер системный
}
//##############################################################################
